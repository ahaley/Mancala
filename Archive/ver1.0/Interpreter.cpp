...
double Interpreter::Interpret_MHP(MHP *mprogram, int program_counter, Mancala *mancala)
{
	if (mprogram->Is_Atom(program_counter))
	{
		...
		return (double)mprogram->Get_Value(program_counter);
	}

	// retrieve the operation at the current program_counter
	int op = mprogram->Get_Value(program_counter);

	// get the pointer to the left argument
	int left_pc = mprogram->Get_Arg(program_counter, 0);
	...
	case MANCALA:
		...
		if ((int)left_value % 2 == 0)	
			return (double)player->get_player_mancala(mancala);
		else
			return (double)player->get_opponent_mancala(mancala);
	...
	case SLOT_PLAYER:
		...
		return (double)player->get_player_count((int)left_value, mancala);
	...
	case SLOT_OPPONENT:
		...
		return (double)player->get_opponent_count((int)left_value, mancala);
