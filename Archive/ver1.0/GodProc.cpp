...
int GodProc::Create_Random_MHP(MHP *mprogram, int depth_counter)
{
	if (depth_counter == 0)
	{
	...
	int function = rand() % NUM_FUNCTION;
	int node = mprogram->Add_Node(function);
...
void GodProc::GP()
{
	...
	for (int g = 0; g < NUMGENERATIONS; g++)
	{
		...
		while (!done)
		{
			do
			{
				random_1 = ((double)rand() / (double)RAND_MAX) * (double)(POOLSIZE - 1.0);
			} while (pool[random_1].state != READY);

			do
			{
				random_2 = ((double)rand() / (double)RAND_MAX) * (double)(POOLSIZE - 1.0);
			} while (pool[random_2].state != READY || random_2 == random_1);

			
			evaluated += 2;
			if (evaluated >= (int)(POOLSIZE - 1))
				done = true;
		...
		while (!done)
		{
			...
			random_node_1 = (int)((rand() / (double)INT_MAX) * (double)pool[random_1].eop);
			random_node_2 = (int)((rand() / (double)INT_MAX) * (double)pool[random_2].eop);
		...
		{
			random_2 = (int)((double)rand() / (double)RAND_MAX * POOLSIZE);
