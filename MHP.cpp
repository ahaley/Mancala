...
short MHP::Add_Node(short value)
{
	...
	{
		cerr << "No more memory, cannot create new nodes!" << endl
			<< "Occurred in short MHP::Add_Node(char value)" << endl;
	}
...
short MHP::Add_SubMHP(short src, MHP *sub_MHP)
{
	...
	if (new_index == -1)
	{
		cerr << "MHP max length reached, aborting.[Add_SubMHP]" << endl;
		exit(0);
	}
